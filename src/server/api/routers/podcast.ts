...
import { podcasts, podcastCharacters, podcastSegments } from "~/server/db/schema"
...
export const podcastRouter = createTRPCRouter({
  ...
  create: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const [podcast] = await ctx.db.insert(podcasts).values({
        title: input.title,
        bookId: input.bookId,
  ...
  delete: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await ctx.db.delete(podcastSegments).where(eq(podcastSegments.podcastId, input.id))
      
      // Delete character associations
      await ctx.db.delete(podcastCharacters).where(eq(podcastCharacters.podcastId, input.id))
